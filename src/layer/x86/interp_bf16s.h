...
static void interp_forward_bf16s_sse(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt, int resize_type, int align_corner, float height_scale, float width_scale, int output_height, int output_width, int has_size_expr)
{
    ...
    if (dims == 2)
    {
        ...
        if (resize_type == 2) // bilinear
        {
            ...
            linear_coeffs(w, outw, xofs, alpha, align_corner, (output_width || has_size_expr) ? (double)w / outw : 1.0 / width_scale);
        ...
        if (resize_type == 3) // bicubic
        {
            ...
            cubic_coeffs(w, outw, xofs, alpha, align_corner, (output_width || has_size_expr) ? (double)w / outw : 1.0 / width_scale);
    ...
    if (resize_type == 2) // bilinear
    {
        ...
        linear_coeffs(w, outw, xofs, alpha, align_corner, (output_width || has_size_expr) ? (double)w / outw : 1.0 / width_scale);
        linear_coeffs(h, outh, yofs, beta, align_corner, (output_height || has_size_expr) ? (double)h / outh : 1.0 / height_scale);
    ...
    if (resize_type == 3) // bicubic
    {
        ...
        cubic_coeffs(w, outw, xofs, alpha, align_corner, (output_width || has_size_expr) ? (double)w / outw : 1.0 / width_scale);
        cubic_coeffs(h, outh, yofs, beta, align_corner, (output_height || has_size_expr) ? (double)h / outh : 1.0 / height_scale);
